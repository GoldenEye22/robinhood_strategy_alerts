...
def database_analysis(path,all_tick,td,decline,skew):
    ...
    for itick in range(len(all_tick)):
        ...
        if len(row) >= td:
            try:
                timediff = datetime.datetime.strptime(row[0]['previous_close_date'], '%Y-%m-%d') - \
                           datetime.datetime.strptime(row[td-1]['previous_close_date'], '%Y-%m-%d')
            except:
            ...
            if timediff.days < 365:
                ...
                try: 
                    if float(row[0]['previous_close'])/float(row[td-1]['previous_close']) < decline:
                        ...
                        /float(row[td-1]['previous_close']))*100)[:4], str(td))
                ...
                try:
                    ...
                    if sentiment >= (1*skew):
                        ...
                        %(all_tick[itick], str(sentiment)[:3], str(td))
                        send = 1
